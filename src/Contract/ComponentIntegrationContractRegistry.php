 ...
 */
final class ComponentIntegrationContractRegistry
{
    /** @var array<string, ComponentIntegrationContractInterface>|null lazy index */
    private ?array $index = null;

    /**
     * @param iterable<ComponentIntegrationContractInterface> $providers
     */
    public function __construct(
    ...
    public function get(string $componentKey): object
    {
        ...
        if (null === $provider) {
            throw new \RuntimeException(\sprintf('No integration contract registered for component "%s". Registered: [%s]. Implement %s and tag it "administering.component_integration_contract".', $componentKey, implode(', ', $this->keys()), ComponentIntegrationContractInterface::class));
        }
    ...
    {
        return array_map(
            static fn (ComponentIntegrationContractInterface $p) => $p->contract(),
            $this->index(),
        );
    }
    ...
    /** @return array<string, ComponentIntegrationContractInterface> */
